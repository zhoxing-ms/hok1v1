...
class Model(nn.Module):
    ...
    def forward(self, data_list, inference=False):
        ...
        # 修改：处理多层LSTM的初始状态
        if lstm_hidden_init is not None and lstm_cell_init is not None:
            # 将初始状态扩展到多层
            lstm_hidden_init = lstm_hidden_init.unsqueeze(0).repeat(self.lstm_num_layers, 1, 1)
            lstm_cell_init = lstm_cell_init.unsqueeze(0).repeat(self.lstm_num_layers, 1, 1)
        else:
            # 如果没有初始状态，创建全零状态
            batch_size = feature_vec.size(0)
            lstm_hidden_init = torch.zeros(self.lstm_num_layers, batch_size, self.lstm_unit_size).to(feature_vec.device)
            lstm_cell_init = torch.zeros(self.lstm_num_layers, batch_size, self.lstm_unit_size).to(feature_vec.device)

        # LSTM前向传播
        lstm_out, (self.lstm_hidden_output, self.lstm_cell_output) = self.lstm(
            reshape_fc_public_result, (lstm_hidden_init, lstm_cell_init)
        )

        # 只使用最后一层的输出
        reshape_lstm_outputs_result = lstm_out.reshape(-1, self.lstm_unit_size)
